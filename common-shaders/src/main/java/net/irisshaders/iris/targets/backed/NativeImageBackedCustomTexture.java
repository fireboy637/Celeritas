...
public class NativeImageBackedCustomTexture extends CommonDynamicTexture implements TextureAccess {
	...
	private static MCNativeImage create(byte[] content) throws IOException {
		ByteBuffer buffer = ByteBuffer.allocateDirect(content.length);
		buffer.put(content);
		buffer.flip();

		return MINECRAFT_SHIM.readNativeImage(buffer);
	}
