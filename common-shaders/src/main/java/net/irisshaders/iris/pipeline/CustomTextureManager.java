...
public class CustomTextureManager {
    protected final EnumMap<TextureStage, Object2ObjectMap<String, TextureAccess>> customTextureIdMap = new EnumMap<>(TextureStage.class);
    protected final Object2ObjectMap<String, TextureAccess> irisCustomTextures = new Object2ObjectOpenHashMap<>();
    protected final TextureAccess noise;
    protected final List<GlTexture> ownedRawTextures = new ArrayList<>();
    /**
	 ...
	 */
	protected final List<MCAbstractTexture> ownedTextures = new ArrayList<>();

    public TextureAccess getNoiseTexture() {
		return noise;
	}

    private TextureAccess createCustomTexture(CustomTextureData textureData) throws IOException, RuntimeException/*ResourceLocationException*/ {
        if (textureData instanceof CustomTextureData.PngData) {
            ...
            return new TextureWrapper(MINECRAFT_SHIM::getLightTextureId, TextureType.TEXTURE_2D);
            ...
            MCTextureManager textureManager = MINECRAFT_SHIM.getTextureManager();

            if (pbrType == null) {
               MCResourceLocation textureLocation = MINECRAFT_SHIM.makeResourceLocation(namespace, location);
                ...
                return new TextureWrapper(() -> {
                    MCAbstractTexture texture = textureManager.getTexture(textureLocation);
                    return texture != null ? texture.getId() : MINECRAFT_SHIM.getMissingTextureId();
                ...
                return new TextureWrapper(() -> {
                    ...
                    if (texture != null) {
                        ...
                        if (textureFormat != null) {
                            int previousBinding = GL_STATE_MANAGER.getBoundTexture(GL_STATE_MANAGER.getActiveTexture());
                    ...
                    return MINECRAFT_SHIM.getMissingTextureId();
                }, TextureType.TEXTURE_2D);
            }
        }
    ...
    public void destroy() {
		ownedTextures.forEach(MCAbstractTexture::close);
    ...
    public CustomTextureManager(PackDirectives packDirectives,
        ...
        customTextureDataMap.forEach((textureStage, customTextureStageDataMap) -> {
            ...
            customTextureStageDataMap.forEach((samplerName, textureData) -> {
                ...
                } catch (IOException | RuntimeException/*ResourceLocationException*/ e) {
